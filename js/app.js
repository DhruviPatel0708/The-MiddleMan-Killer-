...
window.MKApp = {
    ...
    toggleWebsiteTheme() {
        ...
        this.notify(
            'THEME TOGGLED',
            `Switched website to ${this.currentTheme.toUpperCase()} mode`,
            'gold'
        );
    },
    ...
    async loginWithCredentials(identifier, password) {
        ...
        if (loginBtn) {
            loginBtn.innerHTML =
                '<i data-lucide="loader" class="animate-spin"></i> Authenticating...';
        }

        const user = await window.MKAPI.login(
            identifier.trim(),
            password
        );
    ...
    confirmLogout() {
        ...
        this.notify(
            'LOGGED OUT',
            'You have ended your session safely.',
            'gold'
        );
    },
    ...
    renderDynamicNavigation() {
        ...
        if (publicNavMenu) {
            if (role === 'farmer') {
                publicNavMenu.innerHTML = `
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('farmerDashboardView')">
                            <i data-lucide="layout-dashboard"></i>
                            ${i18n ? i18n.getText('navFarmerPortal') : 'Farmer Portal'}
                        </a>
                    </li>

                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('fairPriceRadarView')">
                            <i data-lucide="radar"></i>
                            ${i18n ? i18n.getText('navFairPrice') : 'Fair Price AI'}
                        </a>
                    </li>

                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('marketplaceView')">
                            <i data-lucide="shopping-bag"></i>
                            ${i18n ? i18n.getText('navMarketplace') : 'Marketplace'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('liveAuctionView')">
                            <i data-lucide="zap"></i>
                            ${i18n ? i18n.getText('navAuctions') : 'Live Auctions'}
                        </a>
                    </li>

                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('logisticsView')">
                            <i data-lucide="truck"></i>
                            ${i18n ? i18n.getText('navLogistics') : 'Logistics'}
                        </a>
                    </li>
                `;
            } else if (role === 'buyer') {
                publicNavMenu.innerHTML = `
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('marketplaceView')">
                            <i data-lucide="shopping-bag"></i>
                            ${i18n ? i18n.getText('navMarketplace') : 'Buyer Marketplace'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('escrowPaymentView')">
                            <i data-lucide="lock"></i>
                            ${i18n ? i18n.getText('escrowTag') : 'My Escrow'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('logisticsView')">
                            <i data-lucide="truck"></i>
                            ${i18n ? i18n.getText('navLogistics') : 'Tracking'}
                        </a>
                    </li>
                `;
            } else if (role === 'admin') {
                publicNavMenu.innerHTML = `
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('adminDashboardView')">
                            <i data-lucide="shield-check"></i>
                            ${i18n ? i18n.getText('navAdmin') : 'Admin Command'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('marketplaceView')">
                            ...
                            ${i18n ? i18n.getText('navMarketplace') : 'Crop Management'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('liveAuctionView')">
                            <i data-lucide="zap"></i>
                            ${i18n ? i18n.getText('navAuctions') : 'Auctions'}
                        </a>
                    </li>
                `;
            } else {
                publicNavMenu.innerHTML = `
                    <li>
                        <a class="nav-link active"
                           onclick="MKApp.switchView('landingView')">
                            <i data-lucide="home"></i>
                            ${i18n ? i18n.getText('navHome') : 'Home'}
                        </a>
                    </li>
                    ...
                    <li>
                        <a class="nav-link"
                           onclick="MKApp.switchView('liveAuctionView')">
                            <i data-lucide="zap"></i>
                            ${i18n ? i18n.getText('navAuctions') : 'Live Auction'}
                        </a>
                    </li>
                `;
            }
        ...
        const langSelectorHTML = `
            <select
                class="form-control"
                style="padding: 6px 12px; font-size: 0.82rem; background: rgba(18,55,42,0.8); border: 1px solid var(--border-glow); color: var(--accent-gold); font-weight:700; width:auto; cursor:pointer;"
                onchange="MKI18n.setLanguage(this.value)"
            >
                <option value="en" ${i18n && i18n.currentLang === 'en' ? 'selected' : ''}>
                    🇬🇧 English
                </option>

                <option value="gu" ${i18n && i18n.currentLang === 'gu' ? 'selected' : ''}>
                    🇮🇳 ગુજરાતી
                </option>

                <option value="hi" ${i18n && i18n.currentLang === 'hi' ? 'selected' : ''}>
                    🇮🇳 हिन्दी
                </option>
            </select>
        `;
        ...
        const themeToggleHTML = `
            <button
                ...
                style="padding: 6px 12px; font-size: 0.82rem; background: rgba(18,55,42,0.8); border: 1px solid var(--border-glow); color: var(--accent-gold); font-weight:700; cursor:pointer;"
                onclick="MKApp.toggleWebsiteTheme()"
            ...
            if (this.userSession) {
                ...
                navActions.innerHTML = `
                    ...
                    <button
                        ...
                        onclick="MKApp.toggleSidebar()"
                    >
                        <span>${avatar}</span>

                        <div style="text-align:left;">
                            <div
                                style="font-weight:700; font-size:0.85rem; color:var(--text-ivory);"
                            >
                                ${this.userSession.name}
                            </div>

                            <div
                                style="font-size:0.7rem; color:var(--accent-gold); text-transform:uppercase;"
                            >
                                ${role} ✓
                            </div>
                        </div>
                ...
                navActions.innerHTML = `
                    ...
                    <button
                        ...
                        onclick="MKApp.openModal('authModal')"
                    >
                        <i data-lucide="user"></i>
                        ${i18n ? i18n.getText('navLoginRegister') : 'Login / Register'}
                    </button>
        ...
        if (sidebarMenu && this.userSession) {
            const menuItems =
                window.MKData.roleNavMenus[role] || [];
    ...
    switchView(viewId) {
        ...
        if (
            viewId === 'adminDashboardView' &&
            role !== 'admin'
        ) {
            this.notify(
                'ACCESS DENIED',
                'Admin credentials required to access system command center.',
                'orange'
            );
        ...
        document
            .querySelectorAll('#publicNavMenu .nav-link')
            .forEach(link => {
                const onclickAttr =
                    link.getAttribute('onclick') || '';

                if (
                    onclickAttr.includes(`'${viewId}'`)
                ) {
        ...
        if (
            ...
            viewId === 'fairPriceRadarView' &&
            window.MKAI
        ) {
            window.MKAI.drawFairPriceRadar();
        } else if (
            viewId === 'logisticsView' &&
            window.MKLogistics
        ) {
            window.MKLogistics.initMap();
        }
    },
    ...
    renderRegionalTable() {
        const tbody =
            document.getElementById('regionalTableBody');
    ...
    toggleKrishiChat() {
        const panel =
            document.getElementById('krishiChatPanel');
